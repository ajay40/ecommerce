...
def deletecart(request):
    ct = Cart.objects.filter(id=int(request.GET['id']))
...
def modifycart(request):
    ...
    for p in qt:
        qtt = p
    qt = [q for q in range(1, qtt.tot_qty + 1)]
    oldqty = request.GET["tot_qty"]
    cid = request.GET["id"]
    return render(request, 'modifyqty.html', {"cartid": cid, "pid": x, "qtt": qt, "oq": oldqty})
